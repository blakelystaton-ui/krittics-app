...
export async function generateMovieTrivia(movieTitle: string): Promise<TriviaQuestion[]> {
  try {
    console.log(`[Gemini] Step 1: Calling Gemini API for movie: "${movieTitle}"`);
...
The response must be a valid JSON array matching the provided schema.`;
    ...
    console.log("[Gemini] Step 2: Received response from Gemini");
    ...
    console.log("[Gemini] Step 3: Raw response (first 200 chars):", jsonText.substring(0, 200));

    // Strip markdown code blocks if present (```json ... ```)
    jsonText = jsonText.replace(/```json\s*/g, "").replace(/```\s*/g, "").trim();
    console.log("[Gemini] Step 4: After stripping markdown:", jsonText.substring(0, 200));

    let parsedTrivia: TriviaQuestion[];
    try {
      parsedTrivia = JSON.parse(jsonText) as TriviaQuestion[];
      console.log("[Gemini] Step 5: Successfully parsed JSON, got", parsedTrivia.length, "questions");
    } catch (parseError) {
      console.error("[Gemini] JSON Parse Error:", parseError);
      console.error("[Gemini] Failed to parse text:", jsonText);
