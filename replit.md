...
- **Header Navigation**: Browse, Krossfire, Queue, Insights, and Mission buttons with teal gradient aesthetic. Avatar with dropdown (Help, Sign Out) for logged-in users, sign-in button otherwise, integrated with Replit Auth. Search icon opens movie search dialog with teal gradient styling.
